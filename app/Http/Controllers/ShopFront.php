...
//        $tableProduct = (new ShopProductDescription)->getTable();
//        $tableProductDescription = (new ShopProductDescription)->getTable();
        $tableCategory = (new ShopCategory)->getTable();
        $data = ShopCategory::with('productDescription')->
        leftJoin($tableDescription, $tableDescription . '.category_id', $tableCategory . '.id')->
        where($tableDescription . '.lang', sc_get_locale())->
        get();
        $bloghome = (new ShopNews)
            ->setLimit(sc_config('news_list'))
            ->setPaginate()
            ->getData();
        // Get Project
        $tableProjectCategory = (new CmsCategory)->getTable();
        $tableProjectDescription = (new CmsCategoryDescription)->getTable();
        $projects = CmsCategory::leftJoin($tableProjectDescription, $tableProjectDescription . '.category_id', $tableProjectCategory . '.id')
            ->where($tableProjectDescription . '.lang', sc_get_locale())->where('status', 1)->get();

        // Get Partner

        $partners = DuAn::where('status', 1)->get();
